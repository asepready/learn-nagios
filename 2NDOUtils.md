...
/usr/bin/mysqladmin -u root password 'l<0t@q9k^D!$ko^^im|[0'

# Login
mysql -u root -p'l<0t@q9k^D!$ko^^im|[0'
...
CREATE USER 'ndoutils'@'localhost' IDENTIFIED BY 'P@pangkalpinang#123';
GRANT USAGE ON *.* TO 'ndoutils'@'localhost' IDENTIFIED BY 'P@pangkalpinang#123' WITH MAX_QUERIES_PER_HOUR 0 MAX_CONNECTIONS_PER_HOUR 0 MAX_UPDATES_PER_HOUR 0 MAX_USER_CONNECTIONS 0 ; 
...
echo 'show databases;' | mysql -u ndoutils -p'P@pangkalpinang#123' -h localhost
...
# Initialize Database
cd db/
./installdb -u 'ndoutils' -p 'P@pangkalpinang#123' -h 'localhost' -d nagios
cd .. 
...
sed -i 's/^db_pass=.*/db_pass=P@pangkalpinang#123/g' /usr/local/nagios/etc/ndo2db.cfg
...
echo 'select * from nagios.nagios_logentries;' | mysql -u ndoutils -p'P@pangkalpinang#123'
